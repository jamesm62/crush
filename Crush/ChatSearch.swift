...
//  rate
//
//  Created by James McGivern on 5/6/18.
...
class ChatSearch: UIViewController, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate {
            ...
            if search == "" {
                ...
                query?.findObjectsInBackground(block: { (objects, error) in
                    if var users = objects as? [PFUser] {
                        ...
                        self.searchPeople = unsortedPeople.sorted(by: { (user1, user2) -> Bool in
                        ...
                        for person in self.searchPeople {
                            if useLocationToLoadDistances {
                                if let location = person.value(forKey: "location") as? PFGeoPoint {
                                    ...
                                    self.searchDistances.append(distanceAwayRounded)
                                } else {
                                    self.searchDistances.append(nil)
                                }
                            } else {
                                self.searchDistances.append(nil)
                            }
                            ...
                            self.searchPics.append(UIImage(data: pic2)!)
                            self.searchNames.append(person.value(forKey: "name") as! String)
                            self.searchLasts.append(person.value(forKey: "lastName") as! String)
                            self.searchFullNames.append(person.value(forKey: "fullName") as! String)
                            var gender = "male"
                            ...
                            self.searchGenders.append(gender)
                            self.searchBios.append(person.value(forKey: "bio") as! String)
                            self.searchAges.append(person.value(forKey: "age") as! Int)
                            self.searchUsernames.append(person.username!)
                            
                            
                            let photoFiles = person.value(forKey: "addedPics") as! [PFFile]
                            
                            self.searchAddedPicsArrays.append([])
                            ...
                            for photoFile in photoFiles {
                                ...
                                self.searchAddedPicsArrays[self.searchAddedPicsArrays.count-1].append(photo!)
                            }
                            
                            if let school = person.value(forKey: "school") as? String {
                                self.searchSchools.append(school)
                            } else {
                                self.searchSchools.append(nil)
                            }
                            
                            if let socialMediaAccounts = person.value(forKey: "socialMedia") as? [String] {
                                self.searchSocialMediaAccountsArrays.append(socialMediaAccounts)
                            } else {
                                self.searchSocialMediaAccountsArrays.append([])
                            }
                        }
                        self.tableView.reloadData()
                    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        chatUserAccount = searchPeople[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        } else if searchSchools[indexPath.row] != "" && searchSchools[indexPath.row] != nil {
